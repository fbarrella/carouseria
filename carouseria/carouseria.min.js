...
var arrayParam,carouseriaHeight,carouseriaLoop,carouseriaDirection,carouseriaAutoPlay,carouseriaIndex,carouseriaNav,navContent,indexContent,indexContentChild,autoPlay,slideReady,mainCarousel,innerElement,focusElem,maxElem;function setCarousel(divParam){console.clear();clearInterval(autoPlay);if(indexContent!=null)indexContent.remove();if(navContent!=null)navContent.remove();if(document.getElementById("carouseria")!=null){mainCarousel=document.getElementById("carouseria");carouseriaHeight=divParam.split(" ")[0];carouseriaLoop=("true"==divParam.split(" ")[1]);carouseriaDirection=divParam.split(" ")[2];carouseriaAutoPlay=divParam.split(" ")[3];carouseriaIndex=(divParam.split(" ")[4]).split("|");carouseriaNav=(divParam.split(" ")[5]).split("|");mainCarousel.style.height=carouseriaHeight;if(mainCarousel.querySelector(".carouseria-item")!=null){innerElement=mainCarousel.querySelectorAll(".carouseria-item");for(i=0;i<innerElement.length;i++){innerElement[i].style.display="none";innerElement[i].style.opacity=0;innerElement[i].style.position="relative";innerElement[i].style.left="0px";innerElement[i].style.top="0px";innerElement[i].setAttribute("count",i)}
focusElem=innerElement[0];maxElem=innerElement.length-1;slideReady=!0;if(carouseriaNav[0]=="true")navNative(carouseriaNav[1]);if(carouseriaIndex[0]=="true")visualIndex(innerElement.length,carouseriaIndex[1]);refreshFocus(!1);autoplay(carouseriaAutoPlay)}else{console.log("Não foram detectados itens para exibição.")}}else{console.log("Houve erro na detecção do id \"carouseria\"!")}}
function show(elem,reverse){elem.style.top="0px";elem.style.left="0px";if(!reverse){try{fadeIn(elem);if(carouseriaDirection=="horizontal"){slideH(elem,0,mainCarousel.clientWidth,0,reverse)}else{if(carouseriaDirection=="vertical"){slideV(elem,0,mainCarousel.clientHeight,0,reverse)}else{console.log("Erro na definição da direção do carousel.")}}}catch(err){console.log("Erro detectado: "+err)}
setTimeout(()=>{elem.style.display="block";if(carouseriaIndex[0]=="true")indexChange()},10)}else{try{fadeIn(elem);if(carouseriaDirection=="horizontal"){slideH(elem,0,-(mainCarousel.clientWidth),0,reverse)}else{if(carouseriaDirection=="vertical"){slideV(elem,0,-(mainCarousel.clientHeight),0,reverse)}else{console.log("Erro na definição da direção do carousel.")}}}catch(err){console.log("Erro detectado: "+err)}
setTimeout(()=>{elem.style.display="block";if(carouseriaIndex[0]=="true")indexChange()},10)}}
function hide(elem,reverse){if(!reverse){try{fadeOut(elem);if(carouseriaDirection=="horizontal"){slideH(elem,-(mainCarousel.clientWidth),0,-(mainCarousel.clientWidth),reverse)}else{if(carouseriaDirection=="vertical"){slideV(elem,-(mainCarousel.clientHeight),0,-(mainCarousel.clientHeight),reverse)}else{console.log("Erro na definição da direção do carousel.")}}}catch(err){console.log("Erro detectado: "+err)}
setTimeout(()=>{elem.style.display="none"},300)}else{try{fadeOut(elem);if(carouseriaDirection=="horizontal"){slideH(elem,mainCarousel.clientWidth,0,mainCarousel.clientWidth,reverse)}else{if(carouseriaDirection=="vertical"){slideV(elem,mainCarousel.clientHeight,0,mainCarousel.clientHeight,reverse)}else{console.log("Erro na definição da direção do carousel.")}}}catch(err){console.log("Erro detectado: "+err)}
setTimeout(()=>{elem.style.display="none"},300)}}
function autoplay(param){if(param.split("|")[0]=="true"){if(param.split("|")[1]!=null){autoPlay=setInterval(()=>next(),param.split("|")[1])}}else{clearInterval(autoPlay)}}
function visualIndex(num,setParams){indexContent=document.createElement('div');setParams=="vertical"?indexContent.setAttribute("class","carouseria-index verti"):indexContent.setAttribute("class","carouseria-index hori");indexContent.style.height=mainCarousel.clientHeight+"px";indexContent.style.marginTop="-"+mainCarousel.clientHeight+"px";for(var i=0;i<num;i++){indexContent.innerHTML=indexContent.innerHTML+'<span class="index-item"></span>'}
...
function indexChange(){var count=parseInt(focusElem.getAttribute("count"));for(var i=0;i<indexContentChild.length;i++){indexContentChild[i].style.opacity=0.4;indexContentChild[i].setAttribute('onclick','changeFocus(innerElement['+i+'], false)')}
